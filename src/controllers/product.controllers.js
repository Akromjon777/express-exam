...
import { productsPost, productsPut } from '../validation/validate.js';
const PRODUCTGET = async (req, res, next) => {
   ...
   const subCategoriesFilter = subCategories.filter((e) => e.category_id == categoryId);
...
const PRODUCTPOST = async (req, res, next) => {
   ...
   const newProducts = await write('products.model.json', products).catch((error) =>
      next(new errorHandler(error, 500))
   );
   if (newProducts) {

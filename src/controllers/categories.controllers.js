...
import { catigoriesPost, catigoriesPut } from '../validation/validate.js';
...
const CATEGORIESPOST = async (req, res, next) => {
   ...
   const newCategort = await write('categories.model.json', category).catch((error) =>
      next(new errorHandler(error, 500))
   );
   if (newCategort) {
...
const CATEGORIESPUT = async (req, res, next) => {
   ...
   const newcategory = await write('categories.model.json', category).catch((error) =>
      next(new errorHandler(error, 500))
   );
   if (newcategory) {
      return res.status(200).json({
